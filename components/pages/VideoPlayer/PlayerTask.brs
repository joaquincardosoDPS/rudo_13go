...
sub playContentWithAds()
    ...
    RAF.setContentLength(content.duration_seg)
    ' for generic measurements api
    RAF.enableAdMeasurements(true) 'if unset, ContentNode has it as []
    ...
    if keepPlaying then
        ...
        video.visible = true
        video.control = "play"
        if m.top.isLivePlayer = false then video.setFocus(true) 'so we can handle a Back key interruption
    end if
