...
- [ ] Paso 4c — imágenes de marca definitivas (logo, splash, íconos — medidas exactas de Roku pendientes de verificar).
- [ ] Paso 4d — reescritura de `ContentAPI.brs`/`BaseRequests.brs`: auth y perfiles van por el gateway único (`action`/`path`, ver Paso 3), catálogo por el feed de 13.cl. `BaseRequests.brs` hoy no soporta ese patrón, hay que revisarlo función por función contra `c13_reloaded/src/services/*.ts` y `src/features/auth/services/*.ts`.
